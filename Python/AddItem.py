...
class AddItem:
    ...
    def __init__(self, date=[0, 0, 0]):
        ...
        self.exitBTN = Button(
            "x", 20, Point(screen.getWidth() - 25, 0), Point(screen.getWidth(), 25)
        )
        # self.exitBTN.draw(screen)
        self.submitBTN = Button(
            "Submit",
            15,
            Point(screen.getWidth() - 60, screen.getHeight() - 25),
            Point(screen.getWidth(), screen.getHeight()),
        )
    ...
    def run(self):
        ...
        while running:
            ...
            if (self.submitBTN.pressed(mouseClick) or key == "Return") and (
                len(self.title.getText()) > 1
                and self.subjectList.getSelectedLabel() != ""
                and not "," in self.title.getText()
            ):
                running = False
    ...
    def createOutput(self):
        ...
        myStr += (
            self.dateMonth.getText()
            + ","
            + self.dateDay.getText()
            + ","
            + self.dateYear.getText()
            + ","
        )
