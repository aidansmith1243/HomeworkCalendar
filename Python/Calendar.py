    ...
    def __init__(self, month, day, year, topLeftPoint, bottomRightPoint, row=0):
        ...
        for i in range(self.row - 1, self.row + 5):
            ...
            self.calendar.append(
                Week(tempW, tempD, tempY, month, Point(x1, y1), Point(x2, y2))
            )
...
class Week:
    def __init__(self, month, day, year, viewMonth, topLeftPoint, bottomRightPoint):
        ...
        for i in range(7):
            ...
            x2 += width / 7
            week.append(
                Day(month[i], day[i], year[i], viewMonth, Point(x1, y1), Point(x2, y2))
            )
        self.week = week
...
class Day:
    def __init__(self, month, day, year, viewMonth, topLeftPoint, bottomRightPoint):
        ...
        self.dayLabel = Text(
            Point(bottomRightPoint.getX() - 10, topLeftPoint.getY() + 8), str(day)
        )
        self.list = SelectFromList(
            Point(topLeftPoint.getX(), topLeftPoint.getY() + 15),
        ...
        if self.day == 1:
            self.firstLabel = Text(
                Point(
                    (
                        topLeftPoint.getX()
                        + (bottomRightPoint.getX() - topLeftPoint.getX()) / 2
                    ),
                    topLeftPoint.getY() + 7,
                ),
                PREFS.MONTH_NAMES[month],
            )
        else:
            self.firstLabel = Text(
                Point(
    ...
    def addItem(self, i):
        ...
        if i[0] != "/":
            if (
                self.day == int(i[2])
                and self.month == int(i[1])
                and self.year == int(i[3])
            ):
                try:
                    if i[-1] != "/":
                        ...
                        self.list.add(
                            i[-2], eval(PREFS.CLASSES[i[0]]), i, complete=True
                        )
                except:
