...
class Button:
    def __init__(self,words,size,p1,p2,hasSecondary=False):
        ...
        self.title = Text(Point(self.p1.getX()+self.width/2,self.p1.getY()+self.height/2),self.words)
        self.title.setSize(self.size)

        self.textColor = color_rgb(0,0,0)

        self.useSecondary = hasSecondary
        self.secondaryBtn = Rectangle(Point(self.p1.getX()+1,self.p1.getY()+1),Point(self.p1.getX()+7,self.p2.getY()-1))
...
class SelectFromList:
    ...
    def add(self,label,color,serial=[],complete = False):
        ...
        q = 75#150
        b.setFill(color, color_rgb(80,80,80))
        self.list.append(b)
        ...
        for i in self.list:
            ...
            b.setFill(i.color, color_rgb(80,80,80))
